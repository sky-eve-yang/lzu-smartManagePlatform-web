...
export default {
  name: 'party_history_study_statistics',
  ...
  methods: {
    ...
    formmaterDate(date) {
      ...
      return formattedDate; // 输出： "2023-02"
    },
    getCrudOptions() { 
    ...
    async pageRequest(res) {

      console.log(res)
      // 日期格式化 - 开始
      const formmatedDate = res.meeting_time ? this.formmaterDate(res.meeting_time) : this.formmaterDate()
      res.year_month = formmatedDate
      const query = res
      console.log(query)
      // 日期格式化 - 结束

      const returnData = await api.GetList(query)
      console.info('pageRequest', returnData)
